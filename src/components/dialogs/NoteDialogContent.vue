<template>
  <Dialog class="note-dialog" v-on:accept="accept" v-on:close="close" width="500">
    <v-card>
      ...
      <v-container class="pa-0 px-3 pb-5">
        ...
        <v-autocomplete
          ...
          color="primary"
          multiple
          no-data-text="No tag found. Tags needs to be created before being used."
        ></v-autocomplete>
        <v-select
          class="px-2"
          auto
          v-bind:items="milestoneItems"
          v-model="milestone"
          single-line
          return-object
          required
...
export default {
  ...
  mounted() {
    ...
    this.categories.forEach(category =>
      this.items.push({ text: category.title, tag: category.tag })
    );

    // Setup the milestones
    this.milestones.forEach(milestone =>
      this.milestoneItems.push({ text: `Milestone: ${milestone.title}`, id: milestone.id })
    );
